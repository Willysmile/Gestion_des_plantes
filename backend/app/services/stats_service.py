...
class StatsService:
    ...
    def get_upcoming_predictions(db: Session, days_ahead: int = 7) -> dict:
        """
        Retourne les arrosages et fertilisations PRÉDITS pour les N prochains jours
        Basé sur les prédictions du calendrier (beaucoup plus pertinent!)
        
        Retourne:
        {
            "waterings": [{plant_id, plant_name, predicted_date, days_until}],
            "fertilizings": [{plant_id, plant_name, predicted_date, days_until}],
            "summary": {count_watering, count_fertilizing, most_urgent}
        }
        """
        try:
            today = datetime.now().date()
            ...
            # Récupérer les prédictions des calendriers des jours à venir
            current_date = today
            while current_date <= end_date:
                # Récupérer le calendrier de ce mois
                calendar_events = StatsService.get_calendar_events(db, current_date.year, current_date.month)
                
                # Filtrer les événements pour cette date
                for event in calendar_events["events"]:
                    event_date = datetime.fromisoformat(event["date"]).date()
                    
                    # Vérifier si c'est une prédiction dans la période
                    if today <= event_date <= end_date and event.get("is_predicted"):
                        ...
                        if event["type"] == "watering":
                            waterings.append({
                                ...
                                "last_event_date": event.get("last_watering_date")
                            })
                            ...
                            fertilizings.append({
                                ...
                                "predicted_date": event["date"],
                                "days_until": days_until,
                                "last_event_date": event.get("last_fertilizing_date")
                            })
            ...
            # Dédupliquer (au cas où) et trier par urgence
            waterings = list({(w["plant_id"], w["predicted_date"]): w for w in waterings}.values())
            fertilizings = list({(f["plant_id"], f["predicted_date"]): f for f in fertilizings}.values())
            
            waterings = sorted(waterings, key=lambda x: x["days_until"])
            fertilizings = sorted(fertilizings, key=lambda x: x["days_until"])
            
            # Déterminer la tâche la plus urgente
            most_urgent = None
            if waterings and waterings[0]["days_until"] <= fertilizings[0]["days_until"] if fertilizings else True:
                most_urgent = f"Arroser {waterings[0]['plant_name']} dans {waterings[0]['days_until']} jours"
            elif fertilizings:
                most_urgent = f"Fertiliser {fertilizings[0]['plant_name']} dans {fertilizings[0]['days_until']} jours"
            ...
            print(f"Erreur StatsService.get_upcoming_predictions: {e}")
            import traceback
            traceback.print_exc()
            return {
                "waterings": [],
                "fertilizings": [],
                "summary": {"count_watering": 0, "count_fertilizing": 0, "days_ahead": days_ahead, "most_urgent": None, "total_count": 0}
            }
